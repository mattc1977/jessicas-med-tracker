...
app.get('/api/refills', async (req, res) => {
    ...
    const medsWithDaysOfSupply = allMeds.map(med => {
        let pillsPerDay;
        switch (med.frequency) { case 'tid': pillsPerDay = 3; break; case 'q4h': pillsPerDay = 6; break; default: pillsPerDay = 1; break; }
...
app.post('/api/inventory/update', async (req, res) => {
    ...
    res.status(200).json(await Medication.findOneAndUpdate({ id: medicationId }, { quantity_on_hand: parseInt(newQuantity, 10) }, { new: true }));
});
app.post('/api/refills/received', async (req, res) => {
    ...
    await med.save();
    await Log.create({ type: 'REFILL_RECEIVED', uniqueId: `refill-received-${medicationId}-${new Date().getTime()}`, timestamp: new Date(), medicationId: med.id, name: med.name, details: `Added ${quantity} pills. New total: ${med.quantity_on_hand}.` });
    res.json(med);
});
